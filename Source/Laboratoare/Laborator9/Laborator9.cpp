...
{
	{
		...
		RenderSimpleMesh(meshes["sphere"], shaders["ShaderLab9"], modelMatrix, mapTextures["earth"], NULL, 0, 1);
	}
...

	{
		...
		RenderSimpleMesh(meshes["box"], shaders["ShaderLab9"], modelMatrix, mapTextures["random"], mapTextures["earth"], 1);
	}

	{
		...
		RenderSimpleMesh(meshes["square"], shaders["ShaderLab9"], modelMatrix, mapTextures["grass"], NULL, 0, 0, 1);
	}
...
void Laborator9::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, Texture2D* texture1, Texture2D* texture2, int t2, int p, int grass)
{
	...
	//glm::vec3 rotation = glm::vec3(viewMatrix[4], viewMatrix[5], viewMatrix[6]);


	//glUniform3fv(glGetUniformLocation(shader->program, "rotation"), 1, glm::value_ptr(rotation));
	//glUniform1i(glGetUniformLocation(shader->program, "grass"), grass);

	glUniform1i(glGetUniformLocation(shader->program, "p"), p);
	glUniform1f(glGetUniformLocation(shader->program, "time"), Engine::GetElapsedTime());
	
	glUniform1i(glGetUniformLocation(shader->program, "t2"), t2);
...
Texture2D* Laborator9::CreateRandomTexture(unsigned int width, unsigned int height)
{
	...
	t->Init(textureID, width, height, channels);
	t->Create(data, width, height, channels);
